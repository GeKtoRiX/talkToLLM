...
cleanup_stale_processes() {
  ...
  for port in "${API_PORT}" "${WEB_PORT}"; do
    ...
    [[ -z "${pids}" ]] && continue
    log_line "Port ${port} occupied by stale process(es) [${pids//$'\n'/ }]; cleaning up"
    while IFS= read -r pid; do
      [[ -z "${pid}" ]] && continue
      kill -- "-${pid}" 2>/dev/null || kill "${pid}" 2>/dev/null || true
    done <<<"${pids}"
  done
  # Wait up to 3 s for ports to free
  local deadline=$(( SECONDS + 3 ))
  while (( SECONDS < deadline )); do
    local busy=0
    for port in "${API_PORT}" "${WEB_PORT}"; do
      lsof -nP -iTCP:"${port}" -sTCP:LISTEN -t >/dev/null 2>&1 && busy=1 && break
    done
    [[ "${busy}" -eq 0 ]] && return
    sleep 0.5
  done
...
main() {
  ...
  set_stage "0. Stale process cleanup"
  cleanup_stale_processes
